...
void sort_pairs(void)
{
    ...
    */
    ...
    /* to traverse until the miodpoint */
    int mid = pair_count / 2;
    /* explicitly defining it is a good practice instead of relying on C's behaviour */
    if (pair_count % 2 != 0)
    {
        mid = (pair_count - 1) / 2;
    }

    /* go until the middle and swap the element with corresponding element on the other half */
    for (int i = 0; i < mid; i++)
    {
        /* swapping */
        temp = unsorted[i];
    ...
    {
        temp2 = pairs[i];
        /* in place of current place pair with index as defined in the sorted pair */
        pairs[i] = pairs[unsorted[i].index];
        /* place the crurrent pair in the index's old position */
...
void lock_pairs(void)
{
    /* okay phew - for every pair */
    for (int i = 0; i < pair_count; i++)
    {
        /* if it does not form a cycle */
        if (cyclic_check(pairs[i].winner, pairs[i].loser) == false)
        {
            /* lock it */
...
bool cyclic_check(int candidate_a, int candidate_b)
{
    /* if a locks into b and b locks in to a it is a cycle */
    if (locked[candidate_b][candidate_a])
    {
        return true;
    }
    /* else if b does not lock onto a */
    else
    {
        /* who does lock on to a? */
        for (int i = 0; i < candidate_count; i++)
        {
            /* some ith candidate locks on to a */
            if (locked[i][candidate_a])
            {
                /* if cansdidate b locks on to i who locks on to a - who in turn locks on to b - it is a cycle */
                if (cyclic_check(i, candidate_b))
                {
                    return true;
                }
